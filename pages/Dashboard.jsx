...
export default function Dashboard() {
  const { t } = useLang();
  const { parties, entries, loading, fetchAll, connected } = useStore();
  ...
  const isInitialLoad = loading && parties.length === 0 && entries.length === 0;

  if (isInitialLoad) {
    return <div className="page-enter"><DashboardSkeleton /></div>;
  }
    ...
    <div className="page-enter">
      ...
      <div style={s.pageHeader}>
        ...
        <div style={s.headerActions}>
          <span style={s.syncStatus} title={connected ? 'Live — synced with all employees' : 'Updates may need a manual refresh'}>
            <span style={{ ...s.syncDot, background: connected ? 'var(--green-dark)' : 'var(--text-faint)' }} />
            {connected ? 'Live' : 'Offline'}
          </span>
          <button style={s.refreshBtn} onClick={() => fetchAll()} title="Refresh" disabled={loading}>
